...
const AllBanner = () => {
    ...
    const handleFetch = async () => {
        try {
            const res = await axios.get('https://www.api.panandacademy.hoverbusinessservices.com/api/v1/get-all-banner');
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.delete(`https://www.api.panandacademy.hoverbusinessservices.com/api/v1/delete-banner/${id}`);
    ...
    const handleCheckboxChange = async (id, currentActive) => {
        try {
            const newActiveStatus = !currentActive;
            console.log('status',newActiveStatus)
            await axios.put(`https://www.api.panandacademy.hoverbusinessservices.com/api/v1/update-baner/${id}`, { active: newActiveStatus });
            // Update the local state
            setBanners(prevBanners =>
                prevBanners.map(banner =>
                    banner._id === id ? { ...banner, active: newActiveStatus } : banner
                )
            );
