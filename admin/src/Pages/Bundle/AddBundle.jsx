...
const AddBundle = () => {
  ...
  const fetchAllModes = async (req, res) => {
    try {
      const res = await axios.get('https://www.api.panandacademy.hoverbusinessservices.com/api/v1/get-course-mode')
  ...
  const handleCategoryChange = async (e) => {
    ...
    if (value) {
      try {
        const res = await axios.get(`https://www.api.panandacademy.hoverbusinessservices.com/api/v1/get-courses-by-category/${value}`);
        // console.log(res.data.data)
  ...
  const handleBundleModeChange = (index, e) => {
    ...
    setFormData((prevFormData) => {
      const updatedCourseMode = [...prevFormData.bundleMode];

      if (name === 'modeType') {
        const selectedMode = modes.find(item => item._id === value);
        updatedCourseMode[index] = {
          ...updatedCourseMode[index],
          modeType: selectedMode.name,
  ...
  const addBundleMode = () => {
    setFormData((prevFormData) => ({
      ...prevFormData,
      bundleMode: [...prevFormData.bundleMode, { modeId: '', modeType: '', coursePrice: '', coursePriceAfterDiscount: '', courseDiscountPercent: '', courseLink: '' }]
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://www.api.panandacademy.hoverbusinessservices.com/api/v1/create-bundle', formDataToSend, {
        headers: {
  ...
  const editorConfig = {
    readonly: false,
    height: 400,
    // Add these options if necessary to allow pasting
    toolbarSticky: false,
    clipboard: {
      allowPasteFromWord: true, // Allow paste from Word
      cleanOnPaste: false // Disable cleaning pasted content (if needed)
    },
  };
          ...
          {formData.bundleMode.map((mode, index) => (
            <div key={index} className="col-md-12 row g-3">
              ...
              <div className="col-md-6">
                ...
                <select
                  name='modeType'
                  value={mode._id}  // Bind to _id for accurate selection
              ...
              <button type="button" className="btn btn-danger" onClick={() => removeBundleMode(index)}>Remove Mode</button>
            </div>
          ))}
          {/* {formData.bundleMode.map((mode, index) => (
