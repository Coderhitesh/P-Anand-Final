...
function AllBookRating() {
    ...
    const handleFetch = async () => {
        try {
            const resRatings = await axios.get('https://www.api.panandacademy.hoverbusinessservices.com/api/v1/get-all-book-rating');
            const reverseData = resRatings.data.data.reverse();
            setTeacherRating(reverseData);
            paginateData(reverseData, currentPage);

            const resTeachers = await axios.get('https://www.api.panandacademy.hoverbusinessservices.com/api/v1/get-all-book');
    ...
    const handleFetchCAtegory = async() => {
        try {
            const resCategory = await axios.get('https://www.api.panandacademy.hoverbusinessservices.com/api/v1/get-all-book-category')
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.delete(`https://www.api.panandacademy.hoverbusinessservices.com/api/v1/delete-book-rating/${id}`);
    ...
    const getTeacherNameById = (courseId) => {
        ...
        return foundTeacher ? foundTeacher.bookName : "Unknown Course";
    };
                    ...
                    <tbody>
                        {currentItems.map((rating, index) => (
                            <tr key={rating._id}>
                                ...
                                <td>{getTeacherNameById(rating.bookId)}</td>
                                <td>{rating.rating}</td>
                                <td><Link to={`/edit-book-rating/${rating._id}`} className="bt edit">Edit <i className="fa-solid fa-pen-to-square"></i></Link></td>
